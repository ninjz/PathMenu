...
public class PathMenu: UIView, PathMenuItemDelegate {
    ...
    convenience public init(frame: CGRect!, startItem: PathMenuItem?, items:[PathMenuItem]?) {
        ...
        // Modal tint overlay
        let overlayFrame = self.bounds
        overlayView = UIView(frame: overlayFrame)
        overlayView.alpha = 0
        overlayView.backgroundColor = UIColor.whiteColor()
        overlayView.layer.zPosition = 0

        insertSubview(overlayView, belowSubview: startButton!)
    }
    ...
    public func tintOverlay() {
        overlayView.alpha = 0.95
    }

    public func removeTintOverlay() {
